        ...
        <div class="row g-3 mb-3">
            ...
            <div class="col-auto">
                <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                    <a class="btn btn-primary px-4" href="{{ route('admin.statistics.create') }}"><i
                            class="bi bi-plus-lg me-2"></i>Create Statistic</a>
                </div>
            </div>
        </div><!--end row-->
                            ...
                            <tbody>
                                @foreach ($statistics as $statistic)
                                    <tr>
                                        <td>
                                            @if ($statistic->icon)
                                                <img src="{{ asset('storage/' . $statistic->icon) }}" width="40"
                                                    alt="icon">
                                            @endif
                                        </td>
                                        <td>{{ $statistic->name }}</td>
                                        <td>
                                            {{ $statistic->updated_at->format('M d, H:i A') }}
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.statistics.edit', $statistic->id) }}"
                                                class="btn btn-sm btn-warning">Edit</a>
                                            <button type="button" class="btn btn-sm btn-danger btn-delete"
                                                data-url="{{ route('admin.statistics.destroy', $statistic->id) }}">Delete</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
